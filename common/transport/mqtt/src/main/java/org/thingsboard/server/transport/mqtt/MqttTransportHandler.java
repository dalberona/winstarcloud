 ...
 */
package org.winstarcloud.server.transport.mqtt;
...
import org.winstarcloud.server.common.transport.SessionMsgListener;
...
import org.winstarcloud.server.common.transport.auth.SessionInfoCreator;
...
import org.winstarcloud.server.transport.mqtt.adaptors.MqttTransportAdaptor;
import org.winstarcloud.server.transport.mqtt.adaptors.ProtoMqttAdaptor;
import org.winstarcloud.server.transport.mqtt.session.DeviceSessionCtx;
import org.winstarcloud.server.transport.mqtt.session.GatewaySessionHandler;
import org.winstarcloud.server.transport.mqtt.session.MqttTopicMatcher;
import org.winstarcloud.server.transport.mqtt.session.SparkplugNodeSessionHandler;
import org.winstarcloud.server.transport.mqtt.util.ReturnCodeResolver;
...
import org.winstarcloud.server.transport.mqtt.util.sparkplug.SparkplugRpcResponseBody;
...
import static org.winstarcloud.server.common.transport.service.DefaultTransportService.SESSION_EVENT_MSG_CLOSED;
import static org.winstarcloud.server.common.transport.service.DefaultTransportService.SESSION_EVENT_MSG_OPEN;
import static org.winstarcloud.server.common.transport.service.DefaultTransportService.SUBSCRIBE_TO_ATTRIBUTE_UPDATES_ASYNC_MSG;
import static org.winstarcloud.server.common.transport.service.DefaultTransportService.SUBSCRIBE_TO_RPC_ASYNC_MSG;
import static org.winstarcloud.server.transport.mqtt.util.sparkplug.SparkplugConnectionState.OFFLINE;
import static org.winstarcloud.server.transport.mqtt.util.sparkplug.SparkplugMessageType.NDEATH;
import static org.winstarcloud.server.transport.mqtt.util.sparkplug.SparkplugMetricUtil.getTsKvProto;
import static org.winstarcloud.server.transport.mqtt.util.sparkplug.SparkplugTopicUtil.parseTopicPublish;
...
public class MqttTransportHandler extends ChannelInboundHandlerAdapter implements GenericFutureListener<Future<? super Void>>, SessionMsgListener {
    ...
    private void handleSparkplugPublishMsg(ChannelHandlerContext ctx, String topicName, MqttPublishMessage mqttMsg) {
        ...
        try {
            ...
            ack(ctx, msgId, MqttReasonCodes.PubAck.IMPLEMENTATION_SPECIFIC_ERROR);
            closeCtx(ctx, MqttReasonCodes.Disconnect.IMPLEMENTATION_SPECIFIC_ERROR);
        } catch (AdaptorException | WinstarcloudException | InvalidProtocolBufferException e) {
            log.error("[{}] Failed to process publish msg [{}][{}]", sessionId, topicName, msgId, e);
            sendResponseForAdaptorErrorOrCloseContext(ctx, topicName, msgId);
        }
    }
        ...
        try {
            if (sparkplugSessionHandler == null) {
                ...
                if (sparkplugTopicNode != null) {
                    ...
                    throw new WinstarcloudException("Invalid request body", WinstarcloudErrorCode.BAD_REQUEST_PARAMS);
                }
            }
        } catch (Exception e) {
    ...
    private SparkplugTopic validatedSparkplugTopicConnectedNode(MqttConnectMessage connectMessage) throws WinstarcloudException {
    ...
    public void onToDeviceRpcRequest(UUID sessionId, TransportProtos.ToDeviceRpcRequestMsg rpcRequest) {
        ...
        try {
            ...
            this.sendErrorRpcResponse(deviceSessionCtx.getSessionInfo(), rpcRequest.getRequestId(),
                    WinstarcloudErrorCode.INVALID_ARGUMENTS,
                    "Failed to convert device RPC command to MQTT msg: " + rpcRequest.getMethodName() + rpcRequest.getParams());
        }
    }

    private void handleToSparkplugDeviceRpcRequest(TransportProtos.ToDeviceRpcRequestMsg rpcRequest) throws WinstarcloudException {
        ...
        if (sparkplugSessionHandler.getNodeBirthMetrics().containsKey(tsKvProto.getKv().getKey())) {
            ...
            sendErrorRpcResponse(deviceSessionCtx.getSessionInfo(), rpcRequest.getRequestId(),
                    WinstarcloudErrorCode.BAD_REQUEST_PARAMS, "Failed send To Node Rpc Request: " +
        ...
        cf.addListener(result -> {
            ...
            if (throwable != null) {
                ...
                this.sendErrorRpcResponse(sessionInfo, requestId,
                        WinstarcloudErrorCode.INVALID_ARGUMENTS, " Failed send To Device Rpc Request: " + rpcRequest.getMethodName());
                return;
            }
            if (!isAckExpected(payload)) {
    ...
    public void sendErrorRpcResponse(TransportProtos.SessionInfoProto sessionInfo, int requestId, WinstarcloudErrorCode result, String errorMsg) {
