...
public enum SparkplugMessageType {
	...
	public static SparkplugMessageType parseMessageType(String type) throws WinstarcloudException {
		for (SparkplugMessageType messageType : SparkplugMessageType.values()) {
			if (messageType.name().equals(type)) {
				return messageType;
			}
		}
		throw new WinstarcloudException("Invalid message type: " + type, WinstarcloudErrorCode.INVALID_ARGUMENTS);
	}
