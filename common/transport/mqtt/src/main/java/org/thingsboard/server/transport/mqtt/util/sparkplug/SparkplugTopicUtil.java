...
public class SparkplugTopicUtil {
     ...
     * @throws WinstarcloudException if an error occurs while parsing
     */
    public static SparkplugTopic parseTopicSubscribe(String topic) throws WinstarcloudException {
        // TODO "+", "$"
        topic = topic.indexOf("#") > 0 ? topic.substring(0, topic.indexOf("#")) : topic;
        return parseTopic(SparkplugTopicUtil.getSplitTopic(topic));
    }

    public static SparkplugTopic parseTopicPublish(String topic) throws WinstarcloudException {
        if (topic.contains("#") || topic.contains("$") || topic.contains("+")) {
            throw new WinstarcloudException("Invalid of topic elements for Publish", WinstarcloudErrorCode.INVALID_ARGUMENTS);
        } else {
            String[] splitTopic = SparkplugTopicUtil.getSplitTopic(topic);
            if (splitTopic.length < 4 || splitTopic.length > 5) {
                throw new WinstarcloudException(TOPIC_INVALID_NUMBER + splitTopic.length, WinstarcloudErrorCode.INVALID_ARGUMENTS);
            }
    ...
    public static SparkplugTopic parseTopic(String[] splitTopic) throws WinstarcloudException {
        int length = splitTopic.length;
        if (length == 0) {
			throw new WinstarcloudException(TOPIC_INVALID_NUMBER + length, WinstarcloudErrorCode.INVALID_ARGUMENTS);
        } else {
     ...
     * @return
     */
    private static String validateNameSpace(String nameSpace)  throws WinstarcloudException {
        if (NAMESPACE.equals(nameSpace)) return nameSpace;
        throw new WinstarcloudException("The namespace [" + nameSpace + "] is not valid and must be [" + NAMESPACE + "] for the Sparkplug™ B version.", WinstarcloudErrorCode.INVALID_ARGUMENTS);
    }
