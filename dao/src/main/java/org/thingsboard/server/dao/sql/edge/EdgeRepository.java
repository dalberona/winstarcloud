...
public interface EdgeRepository extends JpaRepository<EdgeEntity, UUID> {
    ...
    @Query("SELECT new org.winstarcloud.server.dao.model.sql.EdgeInfoEntity(d, c.title, c.additionalInfo) " +
            "FROM EdgeEntity d " +
    ...
    @Query("SELECT new org.winstarcloud.server.dao.model.sql.EdgeInfoEntity(a, c.title, c.additionalInfo) " +
            "FROM EdgeEntity a " +
