...
public interface DeviceProfileRepository extends JpaRepository<DeviceProfileEntity, UUID>, ExportableEntityRepository<DeviceProfileEntity> {
    ...
    @Query("SELECT new org.winstarcloud.server.common.data.DeviceProfileInfo(d.id, d.tenantId, d.name, d.image, d.defaultDashboardId, d.type, d.transportType) " +
    ...
    @Query("SELECT new org.winstarcloud.server.common.data.EntityInfo(dp.id, 'DEVICE_PROFILE', dp.name) " +
    ...
    @Query("SELECT new org.winstarcloud.server.common.data.EntityInfo(d.id, 'DEVICE_PROFILE', d.name) " +
            "FROM DeviceProfileEntity d WHERE d.tenantId = :tenantId")
