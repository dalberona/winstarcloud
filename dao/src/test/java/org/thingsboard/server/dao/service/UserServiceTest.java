...
public class UserServiceTest extends AbstractServiceTest {
    ...
    public void before() {
        ...
        tenantAdmin.setEmail("tenant@winstarcloud.org");
        ...
        customerUser.setEmail("customer@winstarcloud.org");
    ...
    public void testFindUserByEmail() {
        User user = userService.findUserByEmail(SYSTEM_TENANT_ID, "sysadmin@winstarcloud.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.SYS_ADMIN, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "tenant@winstarcloud.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.TENANT_ADMIN, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "customer@winstarcloud.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.CUSTOMER_USER, user.getAuthority());
        user = userService.findUserByEmail(tenantId, "fake@winstarcloud.org");
        Assert.assertNull(user);
    }

    @Test
    public void testFindUserByTenantIdAndEmail() {
        User user = userService.findUserByTenantIdAndEmail(SYSTEM_TENANT_ID, "sysadmin@winstarcloud.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.SYS_ADMIN, user.getAuthority());
        user = userService.findUserByTenantIdAndEmail(tenantId, "tenant@winstarcloud.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.TENANT_ADMIN, user.getAuthority());
        user = userService.findUserByTenantIdAndEmail(tenantId, "customer@winstarcloud.org");
        Assert.assertNotNull(user);
        Assert.assertEquals(Authority.CUSTOMER_USER, user.getAuthority());
        user = userService.findUserByTenantIdAndEmail(tenantId, "fake@winstarcloud.org");
        Assert.assertNull(user);
    }
    ...
    public void testSaveUser() {
        User tenantAdminUser = userService.findUserByEmail(SYSTEM_TENANT_ID,"tenant@winstarcloud.org");
        User user = new User();
    ...
    public void testSaveUserWithSameEmail() {
        User tenantAdminUser = userService.findUserByEmail(tenantId, "tenant@winstarcloud.org");
        tenantAdminUser.setEmail("sysadmin@winstarcloud.org");
        Assertions.assertThatThrownBy(() -> userService.saveUser(tenantId, tenantAdminUser))
                .isInstanceOf(DataValidationException.class)
                .hasMessage("User with email 'sysadmin@winstarcloud.org' already present in database!");
    }

    @Test
    public void testSaveUserWithInvalidEmail() {
        User tenantAdminUser = userService.findUserByEmail(tenantId, "tenant@winstarcloud.org");
        tenantAdminUser.setEmail("tenant_winstarcloud.org");
        Assertions.assertThatThrownBy(() -> userService.saveUser(tenantId, tenantAdminUser))
                .isInstanceOf(DataValidationException.class)
                .hasMessage("Invalid email address format 'tenant_winstarcloud.org'!");
    }
    ...
    public void testFindCustomerUsers() {
        User customerUser = userService.findUserByEmail(tenantId, "customer@winstarcloud.org");
