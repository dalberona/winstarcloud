 ...
 */
package org.winstarcloud.server.actors.service;
...
import org.winstarcloud.server.actors.TbActorSystem;
import org.winstarcloud.server.actors.TbActorSystemSettings;
import org.winstarcloud.server.actors.app.AppActor;
import org.winstarcloud.server.actors.app.AppInitMsg;
import org.winstarcloud.server.actors.stats.StatsActor;
...
public class DefaultActorService extends TbApplicationEventListener<PartitionChangeEvent> implements ActorService {
    ...
    private ExecutorService initDispatcherExecutor(String dispatcherName, int poolSize) {
        ...
        if (poolSize == 1) {
            return Executors.newSingleThreadExecutor(WinstarCloudThreadFactory.forName(dispatcherName));
        } else {
            return WinstarCloudExecutors.newWorkStealingPool(poolSize, dispatcherName);
        }
    }
