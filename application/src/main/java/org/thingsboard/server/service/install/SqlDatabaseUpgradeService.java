...
import org.winstarcloud.server.service.install.update.DefaultDataUpdateService;
...
public class SqlDatabaseUpgradeService implements DatabaseEntitiesUpgradeService {
                ...
                updateSchema("3.5.1", 3005001, "3.6.0", 3006000, conn -> {
                    ...
                    try {
                        conn.createStatement().execute("UPDATE rule_node SET " +
                                ...
                                "WHERE type = 'org.winstarcloud.rule.engine.telemetry.TbMsgAttributesNode' AND configuration_version < 1;");
                    } catch (Exception e) {
                    }
                    try {
    ...
    private void loadSql(Path sqlFile, Connection conn) throws Exception {
        ...
        st.execute(sql);//NOSONAR, ignoring because method used to execute winstarcloud database upgrade script
        printWarnings(st);
        Thread.sleep(5000);
    }
