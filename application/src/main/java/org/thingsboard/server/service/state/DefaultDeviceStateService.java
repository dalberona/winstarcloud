...
import org.winstarcloud.server.common.data.exception.TenantNotFoundException;
...
import org.winstarcloud.server.dao.util.DbTypeInfoComponent;
...
import org.winstarcloud.server.service.partition.AbstractPartitionBasedService;
...
import static org.winstarcloud.server.common.data.DataConstants.SCOPE;
...
public class DefaultDeviceStateService extends AbstractPartitionBasedService<DeviceId> implements DeviceStateService {
    ...
    public void init() {
        super.init();
        deviceStateExecutor = MoreExecutors.listeningDecorator(WinstarCloudExecutors.newWorkStealingPool(
                Math.max(4, Runtime.getRuntime().availableProcessors()), "device-state"));
        deviceStateCallbackExecutor = MoreExecutors.listeningDecorator(WinstarCloudExecutors.newWorkStealingPool(
