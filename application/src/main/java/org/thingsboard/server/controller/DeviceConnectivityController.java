...
import org.winstarcloud.server.dao.device.DeviceConnectivityService;
...
import static org.winstarcloud.server.controller.ControllerConstants.PROTOCOL;
import static org.winstarcloud.server.controller.ControllerConstants.PROTOCOL_PARAM_DESCRIPTION;
...
import static org.winstarcloud.server.dao.util.DeviceConnectivityUtil.DOCKER_COMPOSE_YML;
...
public class DeviceConnectivityController extends BaseController {
    ...
    public JsonNode getDevicePublishTelemetryCommands(@Parameter(description = DEVICE_ID_PARAM_DESCRIPTION)
                                                      @PathVariable(DEVICE_ID) String strDeviceId, HttpServletRequest request) throws WinstarcloudException, URISyntaxException {
        checkParameter(DEVICE_ID, strDeviceId);
        DeviceId deviceId = new DeviceId(toUUID(strDeviceId));
        Device device = checkDeviceId(deviceId, org.winstarcloud.server.service.security.permission.Operation.READ_CREDENTIALS);
    ...
    public ResponseEntity<org.springframework.core.io.Resource> downloadServerCertificate(@Parameter(description = PROTOCOL_PARAM_DESCRIPTION)
                                                                                          @PathVariable(PROTOCOL) String protocol) throws WinstarcloudException, IOException {
    ...
    public ResponseEntity<org.springframework.core.io.Resource> downloadGatewayDockerCompose(@Parameter(description = DEVICE_ID_PARAM_DESCRIPTION)
                                                                                             @PathVariable(DEVICE_ID) String strDeviceId, HttpServletRequest request) throws WinstarcloudException, URISyntaxException, IOException {
        checkParameter(DEVICE_ID, strDeviceId);
        DeviceId deviceId = new DeviceId(toUUID(strDeviceId));
        Device device = checkDeviceId(deviceId, Operation.READ_CREDENTIALS);

        if (!checkIsGateway(device)) {
            throw new WinstarcloudException("The device must be a gateway!", WinstarcloudErrorCode.BAD_REQUEST_PARAMS);
        }
