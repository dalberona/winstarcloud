...
import org.winstarcloud.server.service.entitiy.alarm.TbAlarmService;
...
import static org.winstarcloud.server.controller.ControllerConstants.ALARM_INFO_DESCRIPTION;
import static org.winstarcloud.server.controller.ControllerConstants.ASSIGNEE_ID;
import static org.winstarcloud.server.controller.ControllerConstants.ASSIGN_ID_PARAM_DESCRIPTION;
...
public class AlarmController extends BaseController {
    ...
    public Alarm getAlarmById(@Parameter(description = ALARM_ID_PARAM_DESCRIPTION)
                              @PathVariable(ALARM_ID) String strAlarmId) throws WinstarcloudException {
    ...
    @ResponseBody
    public Alarm saveAlarm(@io.swagger.v3.oas.annotations.parameters.RequestBody(description = "A JSON value representing the alarm.") @RequestBody Alarm alarm) throws WinstarcloudException {
    ...
    @ResponseBody
    public Boolean deleteAlarm(@Parameter(description = ALARM_ID_PARAM_DESCRIPTION) @PathVariable(ALARM_ID) String strAlarmId) throws WinstarcloudException {
        checkParameter(ALARM_ID, strAlarmId);
        AlarmId alarmId = new AlarmId(toUUID(strAlarmId));
    ...
    public PageData<AlarmInfo> getAlarms(
    ...
    ) throws WinstarcloudException, ExecutionException, InterruptedException {
        ...
        if (alarmSearchStatus != null && alarmStatus != null) {
            throw new WinstarcloudException("Invalid alarms search query: Both parameters 'searchStatus' " +
                    "and 'status' can't be specified at the same time!", WinstarcloudErrorCode.BAD_REQUEST_PARAMS);
        }
    ...
    public PageData<EntitySubtype> getAlarmTypes(@Parameter(description = PAGE_SIZE_DESCRIPTION, required = true)
                                                 ...
                                                 @RequestParam(required = false) String textSearch,
                                                 @Parameter(description = SORT_ORDER_DESCRIPTION, schema = @Schema(allowableValues = {"ASC", "DESC"}))
                                                 @RequestParam(required = false) String sortOrder) throws WinstarcloudException, ExecutionException, InterruptedException {
