...
export class GatewayConfigurationComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.gatewayConfigGroup.get('winstarcloud.security.password').valueChanges.subscribe(password => {
      if (password && password !== '') {
        this.gatewayConfigGroup.get('winstarcloud.security.username').setValidators([Validators.required]);
      } else {
        this.gatewayConfigGroup.get('winstarcloud.security.username').clearValidators();
      }
      this.gatewayConfigGroup.get('winstarcloud.security.username').updateValueAndValidity({emitEvent: false});
    });

    this.toggleRpcFields(false);

    this.gatewayConfigGroup.get('winstarcloud.remoteConfiguration').valueChanges.subscribe(enabled => {
      if (!enabled) {
    ...
    const checkingDeviceActivityGroup = this.gatewayConfigGroup.get('winstarcloud.checkingDeviceActivity') as FormGroup;
    ...
    const securityGroup = this.gatewayConfigGroup.get('winstarcloud.security') as FormGroup;
    ...
    this.attributeService.getEntityAttributes(entityId, AttributeScope.CLIENT_SCOPE,
      ...
      if (attributes.length) {
        ...
        if (general_configuration) {
          const configObj = {winstarcloud: general_configuration};
          if (configObj.winstarcloud.statistics && configObj.winstarcloud.statistics.commands) {
            for (const command of configObj.winstarcloud.statistics.commands) {
              this.addCommand(command);
            }
            delete configObj.winstarcloud.statistics.commands;
          }
          this.gatewayConfigGroup.patchValue(configObj, {emitEvent: false});
          this.gatewayConfigGroup.markAsPristine();
          if (!configObj.winstarcloud.remoteConfiguration) {
            this.gatewayConfigGroup.disable({emitEvent: false});
          }
          this.checkAndFetchCredentials(configObj.winstarcloud.security);
        }
    ...
    if (security.type !== SecurityTypes.TLS_PRIVATE_KEY) {
      this.deviceService.getDeviceCredentials(this.device.id).subscribe(credentials => {
        ...
        if (credentials.credentialsType === DeviceCredentialsType.ACCESS_TOKEN || security.type === SecurityTypes.TLS_ACCESS_TOKEN) {
          this.gatewayConfigGroup.get('winstarcloud.security.type').setValue(security.type === SecurityTypes.TLS_ACCESS_TOKEN
            ? SecurityTypes.TLS_ACCESS_TOKEN
            : SecurityTypes.ACCESS_TOKEN);
          this.gatewayConfigGroup.get('winstarcloud.security.accessToken').setValue(credentials.credentialsId);
          if(security.type === SecurityTypes.TLS_ACCESS_TOKEN) {
            this.gatewayConfigGroup.get('winstarcloud.security.caCert').setValue(security.caCert);
          }
          ...
          this.gatewayConfigGroup.get('winstarcloud.security.type').setValue(SecurityTypes.USERNAME_PASSWORD);
          this.gatewayConfigGroup.get('winstarcloud.security.clientId').setValue(parsedValue.clientId);
          this.gatewayConfigGroup.get('winstarcloud.security.username').setValue(parsedValue.userName);
          this.gatewayConfigGroup.get('winstarcloud.security.password').setValue(parsedValue.password, {emitEvent: false});
  ...
  commandFormArray(): FormArray {
    return this.gatewayConfigGroup.get('winstarcloud.statistics.commands') as FormArray;
  }
  ...
  private generateLogsFile(logsObj: any) {
    const logAttrObj = {
      ...
      handlers: {
        ...
        databaseHandler: {
          class: 'winstarcloud_gateway.tb_utility.tb_handler.TimedRotatingFileHandler',
          formatter: 'LogFormatter',
  ...
  saveConfig(): void {
    ...
    value.winstarcloud.statistics.commands = Object.values(value.winstarcloud.statistics.commands);
    const attributes = [];
    attributes.push({
  ...
  private openConfigurationConfirmDialog(): void {
    this.deviceService.getDevice(this.device.id).subscribe(gateway => {
      ...
      (GatewayRemoteConfigurationDialogComponent, {
        ...
        (res) => {
          if (!res) {
            this.gatewayConfigGroup.get('winstarcloud.remoteConfiguration').setValue(true, {emitEvent: false});
          }
        }
      );
    });
