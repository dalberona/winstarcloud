...
interface GatewaySetting extends Connector{
  winstarcloud: GatewayMainSetting;
}
...
interface GatewayMainSetting {
  winstarcloud: GatewayMainWinstarcloudSetting;
  connectors: Array<GatewayMainConnector>,
  logs: string,
  storage: GatewayStorage
}

interface GatewayMainWinstarcloudSetting {
  host: string,
  remoteConfiguration: boolean,
  port: number,
...
export function createFormConfig(keyValue: GatewayMainSetting): GatewayFormModels {
  ...
  if (Object.prototype.hasOwnProperty.call(keyValue, 'winstarcloud')) {
    formSetting.host = keyValue.winstarcloud.host;
    formSetting.port = keyValue.winstarcloud.port;
    formSetting.remoteConfiguration = keyValue.winstarcloud.remoteConfiguration;
    if (Object.prototype.hasOwnProperty.call(keyValue.winstarcloud.security, SecurityType.accessToken)) {
      formSetting.securityType = SecurityType.accessToken;
      formSetting.accessToken = (keyValue.winstarcloud.security as SecurityToken).accessToken;
    } else {
      formSetting.securityType = SecurityType.tls;
      formSetting.caCertPath = (keyValue.winstarcloud.security as SecurityCertificate).caCert;
      formSetting.privateKeyPath = (keyValue.winstarcloud.security as SecurityCertificate).privateKey;
      formSetting.certPath = (keyValue.winstarcloud.security as SecurityCertificate).cert;
    }
  }
...
export function gatewayConfigJSON(gatewayConfiguration: GatewayFormModels): GatewaySetting {
  const gatewayConfig = {
    winstarcloud: gatewayMainConfigJSON(gatewayConfiguration)
  };
...
function gatewayMainConfigJSON(gatewayConfiguration: GatewayFormModels): GatewayMainSetting {
  ...
  const winstarcloud: GatewayMainWinstarcloudSetting = {
